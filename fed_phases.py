...
def process_log_file(filepath, run_outputs, group_by_run, trans_dat, filter_state, run_summary, total_summary):
            ...
            while i < len(lines):
                ...
                if len(parts) >= 6:
                    ...
                    if state in ("Weak", "Dead"):
                        ...
                        if fed_number and fiber_number.isdigit() and detector:
                            key = (fed_number, fiber_number)
                            run_key = current_run if current_run else "No Run Number Found"

                            if run_key not in run_summary:
                                run_summary[run_key] = {}

                            if key not in run_summary[run_key]:
                                run_summary[run_key][key] = (state, detector, rocs)

                            if key not in total_summary:
                                total_summary[key] = (state, detector, rocs)     
...
def build_summary(summary_dict):
    result = {}

    for (_, _), (state, detector, rocs) in summary_dict.items():
        if detector not in result:
            result[detector] = {"Weak": 0, "Dead": 0, "sub": {}}

        result[detector][state] += 1
        ...
        if subdet not in result[detector]["sub"]:
            result[detector]["sub"][subdet] = {"Weak": 0, "Dead": 0}

        result[detector]["sub"][subdet][state] += 1
...
def print_summary(write_and_print, title, summary_dict):
    data = build_summary(summary_dict)

    write_and_print("\n=========================================================")
    write_and_print(title)
    write_and_print("=========================================================")
    write_and_print("Detector     |  Weak |  Dead | Total")
    write_and_print("---------------------------------------------------------")

    total_w = 0
    total_d = 0

    for det in sorted(data.keys()):
        w = data[det]["Weak"]
        d = data[det]["Dead"]
        total = w + d
        ...
        write_and_print(f"{det:<13}|{w:6} |{d:6} |{total:6}")
        
        for sub in sorted(data[det]["sub"].keys(), key=sub_sort_key):
            sw = data[det]["sub"][sub]["Weak"]
            sd = data[det]["sub"][sub]["Dead"]
            st = sw + sd
            write_and_print(f"  {sub:<11}|{sw:6} |{sd:6} |{st:6}")

    write_and_print("---------------------------------------------------------")
    write_and_print(f"TOTAL        |{total_w:6} |{total_d:6} |{total_w + total_d:6}")
    write_and_print("=========================================================")
...
def main(directory, show_run, save_output, filter_state):
    ...
    for root, _, files in os.walk(directory):
        for file in files:
            if "PixelFEDSupervisor" in file and file.endswith(".log"):
                ...
                process_log_file(filepath, run_outputs, show_run, trans_dat, filter_state, run_summary, total_summary)
    ...
    if show_run:
        for run_number in sorted(
            run_outputs.keys(), key=lambda x: (x == "No Run Number Found", int(x) if x.isdigit() else 0)
        ):
    ...
    # for run_number in sorted(
    #     run_summary.keys(), key=lambda x: (x == "No Run Number Found", int(x) if x.isdigit() else 0)
    # ):
    #     print_summary(
    #         write_and_print,
    #         f"SUMMARY OF DEAD/WEAK FIBERS (Run {run_number})",
    #         run_summary[run_number],
    #     )

    print_summary(
        write_and_print,
        "SUMMARY OF DEAD/WEAK FIBERS (TOTAL)",
        total_summary,
    )
